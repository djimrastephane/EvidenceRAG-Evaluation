...
```bash
DEMO_MODE=1 UI_DATA_ROOT=results/table_rechunk_ablation_2026-03-22/artifacts/row_blocks python -m uvicorn app.api.main:app --reload --port 8000
```

UI:

```bash
DEMO_MODE=1 UI_DATA_ROOT=results/table_rechunk_ablation_2026-03-22/artifacts/row_blocks python -m streamlit run app/ui/streamlit_app.py --server.port 8502
```
