...
|---|---|---|---|
| 1. Chunking selection | Historical sweep compared `280/90` vs `320/90` and `seg_on/off`; current thesis pipeline uses `224/56`, `seg_off` | Current thesis retrieval configuration is fixed-window `224/56` with `seg_off`. Older `280/90` selection files are retained as historical ablation records. | Historical sweep not formally tested |
