#!/usr/bin/env python3
"""E5-Financial encoder ablation: dense-only and hybrid, subsection boost OFF.

Uses FinLang/finance-embeddings-investopedia (E5-base fine-tuned on Investopedia
financial text, 768-dim) to extend Table C.3. Same data_processed/ chunks,
eval_set.json, RRF/BM25 settings, and no subsection boosting as the original
dense encoder ablation.
"""
...
OUTPUT_DIR = ROOT / "results" / "e5_financial_encoder_ablation_2026-04-24"
...
def main() -> None:
    ...
    # Allow HF cache access for loading the cached model
    os.environ.pop("TRANSFORMERS_OFFLINE", None)
    os.environ.pop("HF_HUB_OFFLINE", None)

    print(f"Loading {MODEL_ID}...")
    model = SentenceTransformer(MODEL_ID)
    print(f"Loaded (dim={EMBED_DIM}, mean pooling, L2 normalised)\n")
    ...
    for doc_id in DOC_IDS:
        ...
        chunk_vecs = embed(model, [c.text for c in chunks])
        faiss_idx = faiss.IndexFlatIP(chunk_vecs.shape[1])
        faiss_idx.add(chunk_vecs)
        q_vecs = embed(model, [q.query_text for q in queries])
    ...
    dense_margin_val = mean_margin(np.vstack(all_dense_scores))
    ...
    print(f"\n=== E5-Financial Encoder Ablation (n=250 queries) ===")
    hdr = f"{'Setup':<10} {'Hit@1':>7} {'Hit@3':>7} {'MRR@10':>8} {'FP2':>5} {'Margin':>9} {'Runtime(s)':>11}"
    print(hdr)
    print(f"{'dense':<10} {dense_m['hit@1']:>7.3f} {dense_m['hit@3']:>7.3f} {dense_m['mrr']:>8.3f} {dense_fp2:>5} {dense_margin_val:>9.4f} {rt_dense:>11.1f}")
    ...
    results = {
        ...
        "dense": {
            ...
            "fp2_count": dense_fp2,
            "mean_margin": round(dense_margin_val, 6),
