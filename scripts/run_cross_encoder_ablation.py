"""
Cross-encoder reranker ablation using ms-marco-MiniLM-L-6-v2.

Runs the existing hybrid evaluation pipeline across all five Grampian documents
with the cross-encoder enabled at varying top-N depths and fusion weights.
Compares against the hybrid base (no cross-encoder) to establish whether
annotation-free reranking can close the FP2 gap.

The oracle subsection boost upper bound (+0.092 Hit@1, thesis Table 4.2) is not
re-run here: chunk_meta.parquet in data_processed has no subsection_title
column, so the boost cannot fire. The thesis oracle result serves as the
annotation-dependent ceiling for comparison.

Configurations tested
---------------------
baseline      : hybrid, no cross-encoder, no subsection boost
ce_topn5_w02  : cross-encoder top-5,  weight=0.2
ce_topn10_w02 : cross-encoder top-10, weight=0.2
ce_topn20_w02 : cross-encoder top-20, weight=0.2  (primary comparison)
ce_topn20_w01 : cross-encoder top-20, weight=0.1
ce_topn20_w03 : cross-encoder top-20, weight=0.3

Results are written to results/cross_encoder_ablation_<date>/.
"""
...
# Use the rag-pipeline conda env that has all required packages
...
CE_MODEL = "models/cross-encoder-ms-marco-MiniLM-L-6-v2"
...
def build_cmd(data_dir: Path, cfg: dict) -> list[str]:
    # Keep lexical rerank on — matches the thesis hybrid base (0.752)
    cmd = [
...
def run_config(cfg_name: str, cfg: dict, out_dir: Path) -> dict:
    """Run one config across all documents and return aggregated metrics."""
    results_by_doc = {}
    for doc_id in DOC_IDS:
        ...
        # Save a per-doc copy so configs don't overwrite each other
...
def aggregate(results_by_doc: dict) -> dict:
    """Average hit@k and MRR across documents, skipping failures.

    Metrics are stored under metrics_by_k[str(k)]['page_hit_rate_at_k'] and
    metrics_by_k[str(k)]['mean_page_mrr_at_k'].
    """
...
def main() -> None:
    ...
    out_dir = ROOT / "results" / f"cross_encoder_ablation_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"Output directory: {out_dir}")
    print(f"Cross-encoder model: {CE_MODEL}")
    ...
    summary_df.to_csv(out_dir / "cross_encoder_ablation_summary.csv", index=False)
    with (out_dir / "cross_encoder_ablation_results.json").open("w") as f:
