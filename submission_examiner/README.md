...
For the thesis refactor configuration, embedding normalization is pipeline-controlled rather than model-controlled. When `embedding.apply_l2_normalization: true`, the pipeline applies an explicit row-wise L2 normalization step after raw embeddings are produced by `all-MiniLM-L6-v2`; this normalization is not performed by MiniLM itself.
