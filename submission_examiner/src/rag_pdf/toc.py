...
def normalize_title(s: str) -> str:
    """Return a lowercased, punctuation-stripped form of a section title for fuzzy comparison."""
...
def detect_toc_pages(pages_df: pd.DataFrame) -> list[int]:
    """Score each page for Table-of-Contents characteristics and return the PDF page numbers that qualify.

    A page accumulates score for: containing the word "contents" near the top, a high density
    of lines ending in a page number, and a high count of numbered list items.  Pages above the
    score threshold are returned as seed pages; adjacent pages that score above a lower threshold
    are also included to capture multi-page ToCs.
    """
...
def parse_toc(pages_df: pd.DataFrame, toc_page_indices: list[int]) -> list[dict[str, Any]]:
    """Parse ToC pages into a list of entry dicts with title, printed page number, and heading level.

    Handles two common ToC layouts: (1) each visual row is a title followed immediately by a
    page number, and (2) title and page number appear on separate lines.  Wrapped multi-line
    titles are concatenated, and unlabeled major headings that lack an explicit page number have
    their page inferred from adjacent entries.  Duplicate entries (same normalised title + page)
    are removed while preserving document order.
    """
...
def resolve_printed_to_pdf_offset(pages_df: pd.DataFrame, toc_items: list[dict[str, Any]]) -> dict[str, Any]:
    """Estimate the offset between printed page numbers (as shown in the document) and 1-based PDF page indices.

    For each candidate ToC entry, searches the pages DataFrame for the page whose text best
    matches the entry title.  The most common (printed → PDF) delta across candidates is returned
    as the offset, together with a confidence score and the number of supporting entries.
    """
    if not toc_items:
...
def build_toc_spans(toc_items: list[dict[str, Any]], offset: int, n_pdf_pages: int) -> list[dict[str, Any]]:
    """Convert ToC entries into page-range spans suitable for annotating chunk section/subsection context.

    Each span covers [start_pdf_page, end_pdf_page] (inclusive), computed by applying the
    printed-to-PDF offset and using the next entry's start page as the current entry's end.
...
def toc_prior_for_page(spans: list[dict[str, Any]], page_no: int) -> tuple[str, str]:
    """Return (section_title, subsection_title) for the given PDF page number using prebuilt ToC spans.

    Returns ('Unknown', 'Unknown') when no span covers the page.
    """
    for sp in spans:
...
def token_set_overlap_ratio(a: str, b: str) -> float:
    """Return the Jaccard overlap of lowercased alphanumeric tokens between two title strings."""
...
def fuzzy_match_title(
    ...
    """Check whether a proposed title fuzzy-matches any entry in an allow-list.

    Uses both token-set Jaccard overlap and SequenceMatcher ratio; a match is declared if
    either score clears its respective threshold.  Returns (matched, best_candidate, best_score).
    """
