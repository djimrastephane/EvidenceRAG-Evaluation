...
def extract_page_struct_hybrid(
    ...
    def run_pymupdf() -> dict:
        """Extract page structure with PyMuPDF."""
        page = doc.load_page(page_index)
        return extract_page_struct_pymupdf(page)

    def run_pdfplumber() -> dict:
        """Extract page structure with pdfplumber."""
        pl_page = pdf_plumber.pages[page_index]
        return extract_page_struct_pdfplumber(pl_page)

    def run_primary() -> tuple[dict, str]:
        """Run the configured primary extractor and tag the result with its name."""
        if primary == "pymupdf":
            return run_pymupdf(), "pymupdf"
        return run_pdfplumber(), "pdfplumber"

    def run_fallback() -> tuple[dict, str]:
        """Run whichever extractor was not chosen as primary."""
        if primary == "pymupdf":
            return run_pdfplumber(), "pdfplumber"
        return run_pymupdf(), "pymupdf"

    def maybe_use_ocr(s_base: dict, used_base: str, note_base: str) -> tuple[dict, str, str]:
        """Upgrade the page struct to OCR output when the parser-extracted text is too short.

        Applies OCR only when Tesseract is available and the combined parser text is under
        50 characters.  The OCR result is validated for quality before replacing the
        parser output; rejected OCR is noted in the quality string but the parser result
        is kept unchanged.
        """
