...
This module keeps embedding behaviour explicit: raw vectors come from the
sentence-transformer model, while any L2 normalization is applied by the
pipeline afterwards. That separation is important for reproducibility and for
documenting ablations cleanly in the thesis.
"""
...
def load_embedding_model(config: EmbeddingConfig, device: str, cache_dir: str) -> SentenceTransformer:
    """Load the configured sentence-transformer model in offline-first mode."""
...
def build_embedding_text(chunk: ChunkRecord) -> str:
    """Construct the text payload embedded for a chunk, including heading context."""
    parts: list[str] = []
...
def embed_chunks(
    ...
    *,
    ...
    """Embed chunk records, validate dimensions, and optionally L2-normalize."""
    ...
    if config.apply_l2_normalization:
...
def embed_queries(
    ...
    *,
    ...
    """Embed query texts using the same model and normalization policy as chunks."""
    model = load_embedding_model(config, device=device, cache_dir=cache_dir)
    vectors = model.encode(
        query_texts,
        batch_size=config.batch_size,
