...
"""Per-query diagnostic record construction.

Diagnostics are a first-class thesis output because they make retrieval errors
inspectable at the query level. This module assembles dense, sparse, hybrid,
and evaluation signals into a single structured view that can be exported to
CSV and reviewed during ablation analysis.
"""
...
def build_query_diagnostics(
    ...
    """Assemble one diagnostic record per query from saved retrieval outputs."""
...
def save_diagnostics_csv(diagnostics: list[QueryDiagnostics], out_path) -> None:
    """Export diagnostics to a flat CSV for examiner-friendly inspection."""
...
def _group_hits(hits: list[RetrievalHit]) -> dict[str, list[RetrievalHit]]:
    """Group hits by query id while preserving their input order."""
