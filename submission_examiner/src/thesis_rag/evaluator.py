...
"""Page-level retrieval evaluation metrics.

Retrieval operates over chunks but the thesis evaluates against gold pages. The
functions here therefore assume page-ranked inputs and compute the transparent,
auditable metrics used throughout the experiments.
"""
...
def hit_at_k(predicted_pages: list[int], gold_pages: list[int], k: int) -> bool:
    """Return whether any gold page appears within the first ``k`` predictions."""
...
def reciprocal_rank(predicted_pages: list[int], gold_pages: list[int]) -> tuple[float, int | None]:
    """Return reciprocal rank and the matching rank of the first gold page."""
    gold = set(gold_pages)
...
def infer_failure_type(gold_pages: list[int], predicted_pages: list[int]) -> str | None:
    """Label simple failure modes to support error analysis exports."""
    if not gold_pages:
...
def evaluate_page_hits(queries: list[QueryRecord], page_hits: list[RetrievalHit]) -> list[EvaluationResult]:
    """Evaluate page-ranked hits against the gold query set."""
...
def aggregate_metrics(results: list[EvaluationResult], ks: list[int]) -> dict[str, float]:
    """Aggregate per-query evaluation records into corpus-level metrics."""
