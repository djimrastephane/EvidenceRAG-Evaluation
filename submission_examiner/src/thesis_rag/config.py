...
"""Configuration loading for the thesis_rag pipeline.

Configuration is YAML-driven so experiments can be rerun and audited from a
single file. This module converts nested YAML payloads into typed dataclasses,
preserves backward compatibility for older config keys, and resolves relative
paths against the config file location rather than the caller's working
directory.
"""
...
def _coerce_value(field_type: Any, value: Any, *, base_dir: Path) -> Any:
    """Coerce raw YAML values into typed dataclass field values."""
...
def _build_dataclass(cls: type[T], payload: dict[str, Any], *, base_dir: Path) -> T:
    """Recursively build a dataclass instance from a nested mapping."""
    template = cls()
    kwargs: dict[str, Any] = {}
    if cls is EmbeddingConfig and "normalize_embeddings" in payload and "apply_l2_normalization" not in payload:
        payload = dict(payload)
        payload["apply_l2_normalization"] = payload["normalize_embeddings"]
...
def _resolve_paths(config: PipelineConfig, *, config_path: Path) -> PipelineConfig:
    """Resolve relative filesystem paths after the dataclass is constructed."""
...
def load_config(config_path: str | Path) -> PipelineConfig:
    """Load a YAML pipeline config into a fully typed ``PipelineConfig``."""
