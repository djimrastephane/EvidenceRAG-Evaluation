...
"""Document discovery and page extraction.

The thesis corpus consists of public-sector PDF reports. This module discovers
those reports from the configured data directory and delegates page extraction
to the legacy hybrid extractor that was retained for parity with the original
pipeline.
"""
...
def discover_documents(data_dir: Path) -> list[DocumentRecord]:
    """Enumerate PDF documents in the configured corpus directory."""
...
def extract_page_structures(document: DocumentRecord) -> list[tuple[int, dict, str, str]]:
    """Extract structured page payloads for one PDF document."""
