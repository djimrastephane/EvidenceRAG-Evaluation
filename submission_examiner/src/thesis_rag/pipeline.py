...
"""Top-level orchestration for the thesis_rag pipeline.

Each public function in this module corresponds to one CLI stage. The design is
deliberately stage-oriented so runs can be reproduced, ablated, and inspected
without notebook state or hidden side effects.
"""
...
def prepare_run(config: PipelineConfig, prefix: str) -> Path:
    """Create a fresh run directory, configure logging, and save the config."""
...
def preprocess_corpus(config: PipelineConfig) -> Path:
    """Run corpus preprocessing and persist page/chunk artifacts per document."""
...
def build_indexes(config: PipelineConfig, chunks_path: Path) -> Path:
    """Embed chunk text, build the exact FAISS index, and save index artifacts."""
...
def retrieve_queries(config: PipelineConfig, chunk_metadata_path: Path, faiss_index_path: Path, query_set_path: Path) -> Path:
    """Run dense, sparse, and hybrid retrieval and write ranked page-hit files."""
...
def evaluate_retrieval(
    ...
    """Evaluate saved retrieval outputs and export metrics plus diagnostics."""
...
def _validate_queries(queries, chunks) -> None:
    """Fail fast if the query set references documents or pages not in the corpus."""
...
def _load_hits_csv_or_jsonl(path: Path):
    """Load retrieval hits from either the JSONL base path or an explicit CSV path."""
    import pandas as pd
