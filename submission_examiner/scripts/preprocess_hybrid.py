...
def _parse_args() -> argparse.Namespace:
    repo_root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(description="Hybrid PDF preprocessing pipeline.")
    parser.add_argument("--config", default=None, help="Path to YAML config (optional).")
    parser.add_argument(
        "--pdf_path", required=True,
        help="Path to the PDF file to preprocess.",
    )
    parser.add_argument(
        "--doc_id", default=None,
        help="Document ID (defaults to PDF filename stem).",
    )
    parser.add_argument(
        "--out_root", default=str(repo_root / "data_processed"),
        help="Output root directory (default: <repo>/data_processed).",
    )
    parser.add_argument("--corpus_id", default=None)
    parser.add_argument("--chunk_size_tokens", type=int, default=224)
    parser.add_argument("--chunk_overlap_tokens", type=int, default=56)
...
def main() -> None:
    ...
    # TableExtractConfig fields are accessed via __getattr__ and must be set
    # on the nested sub-config, not passed to PreprocessConfig.__init__().
    cfg.TABLE_EXTRACT.CAMELOT_LATTICE_ACCURACY_THRESHOLD = 70
    cfg.TABLE_EXTRACT.TABLE_SUMMARY_MAX_ROWS = 5
    ...
    doc_id = args.doc_id or cfg.PDF_PATH.stem
    ...
    with pdfplumber.open(str(cfg.PDF_PATH)) as pdf_plumber:
        ...
        for i in range(doc.page_count):
            ...
            s, used, note = extract_page_struct_hybrid(doc, pdf_plumber, i, pdf_path=str(cfg.PDF_PATH))
        ...
        print("\nExtracting tables...")
        table_chunks_df, structured_tables_df, _table_facts_df, _rejected = process_table_pages(
            table_pages,
            cfg.PDF_PATH,
            pdf_plumber,
            doc_id,
