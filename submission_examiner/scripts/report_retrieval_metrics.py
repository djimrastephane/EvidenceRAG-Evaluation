"""
Aggregate retrieval outputs into thesis-ready summary reports.

This script reads per-query evaluation artifacts and exports compact metric
tables, query-level reports, and failure summaries so that retrieval behaviour
can be analysed consistently across documents and experiments.
"""
...
def resolve_output_path(data_root: Path, output_path: str | None, filename: str) -> Path:
    """Return an explicit output path if provided, otherwise fall back to data_root/filename."""
    if output_path:
...
def main() -> None:
    """Aggregate retrieval_metrics.json files across documents and print a formatted summary table."""
    args = parse_args()
