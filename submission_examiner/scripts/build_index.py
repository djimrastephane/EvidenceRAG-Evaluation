...
# CONFIG
# =============================================================================
BASE_DATA_DIR = Path(__file__).resolve().parent.parent / "data_processed"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build embeddings and FAISS indexes for every document folder under a base data directory."
    )
    parser.add_argument(
        "--data-dir",
        default=str(BASE_DATA_DIR),
        help="Base directory containing document subfolders with chunks.parquet.",
    )
    parser.add_argument(
        "--model",
...
# PER-DOCUMENT BUILD
# =============================================================================
def build_index_for_doc(doc_dir: Path, model: SentenceTransformer, embed_model_name: str = EMBED_MODEL_NAME) -> None:
    """
...
# =============================================================================
def main():
    """Embed chunks with MiniLM-L6-v2, build a FAISS IndexFlatIP, and write the index artefacts to disk."""
    args = parse_args()
    base_data_dir = Path(args.data_dir).resolve()
    embed_model_name = str(args.model)
    ...
    for doc_dir in doc_dirs:
        ...
        try:
            build_index_for_doc(doc_dir, model, embed_model_name)
            processed += 1
