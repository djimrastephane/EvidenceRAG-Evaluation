...
def l2_normalize(x: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """L2-normalise each row of a 2-D array, guarding against zero-norm rows."""
...
def trust_from_doc_id(doc_id: str) -> str:
    """Map a doc_id prefix (e.g. 'Grampian') to its canonical NHS trust name."""
...
def load_doc_artifacts(doc_dir: Path) -> Optional[tuple[pd.DataFrame, pd.DataFrame, np.ndarray]]:
    """Load chunk metadata, chunk parquet, and embeddings array from a processed document directory."""
...
def main() -> None:
    """Concatenate per-document embeddings and build a single global FAISS index and chunk_meta parquet."""
    args = parse_args()
