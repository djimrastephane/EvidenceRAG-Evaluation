"""
Run the full examiner pipeline end to end for one or more documents.

This script orchestrates the main thesis pipeline stages in sequence:
preprocessing, index construction, retrieval evaluation, and report export. It
is intended as the high-level runner for rebuilding the canonical examiner
artifacts from source PDFs and evaluation sets.
"""
...
def run_cmd(cmd: list[str], log_path: Optional[Path] = None) -> None:
    """Run a subprocess command, optionally capturing stdout/stderr to a log file; raise on non-zero exit."""
    if log_path:
...
def main() -> None:
    """Preprocess all PDFs in a directory then build FAISS indexes and evaluate retrieval for each document."""
    args = parse_args()
